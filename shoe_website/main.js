...
(function () {
  ...
  const STORAGE = {
    ...
    adminSession: "dhankor_admin_session_v1"
  };
 ...
 const PRODUCTS = [
  ...
  async function apiRequest(path, options = {}) {
    ...
    if (!response.ok) {
      ...
      try {
        ...
        // Ignore json parsing errors from backend.
      }
  ...
  async function updateProductOnServer(productId, payload) {
    const updated = await apiRequest(`/products/${productId}`, {
      method: "PATCH",
      headers: {
    ...
    if (index !== -1) {
      currentProducts[index] = { ...currentProducts[index], ...updated };
    }
  }
  ...
  function renderProducts() {
    ...
    const cardsHtml = currentProducts.map((product, index) => {
      ...
      if (isAdminLoggedIn) {
        ...
        `;
        priceDisplay = `<span class="z-price z-admin-price-edit" data-admin-price data-product-id="${product.id}">₹${product.price}<span class="z-admin-edit-label" aria-hidden="true">Edit</span></span>`;
      }
        ...
        if (priceBtn) {
          ...
          if (currentProd) {
            ...
            if (newPrice !== null && !isNaN(newPrice) && Number(newPrice) > 0) {
               updateProductOnServer(pId, { price: Number(newPrice) })
                 .then(() => {
                   renderProducts();
                   setupReveal();
                 })
  ...
  async function initStore() {
    try {
      ...
      window.alert("Could not load products from server.");
